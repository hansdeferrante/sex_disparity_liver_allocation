...
This completes the simulated patient data. For each patient, we have:
  ...
  4) a candidate sex,
  5) a candidate blood group
  6) a candidate country of listing
...
on ET registry data. For the purpose of illustration,
we exaggerated the effect of female sex (the real coefficient intercept and
coefficient on female sex are different).
...
acceptance_model_formula <- ~ candidate_sex + meld_lab + f_over_20(meld_lab) + f_over_30(meld_lab)
fixed_effects <- c(
  `(Intercept)` = -1,
  `candidate_sexFemale` = -1.5,
  `meld_lab` = -0.0133,
  `f_over_20(meld_lab)` = 0.103,
  `f_over_30(meld_lab)` = -0.0766
  )
...
In observed waitlist outcomes, there is a large sex disparity with females being less likely to be transplanted and more likely to die on the waiting list.
...
which makes transplantation an independent censoring mechanism. Consequently, the Cox model can recover the true model for medical urgency. In this true model, sex indeed has no effect on pre-transplant survival.

By making females less likely to accept liver offers than males, we have
simulated a dataset with a large disparity in waitlist outcomes. However, these
disparities are not captured by a Cox proportional hazards model which uses
waitlist mortality as the outcome, because such a model recovers the true model
for medical urgency (and not observed waitlist deaths, which are determined not only by medical urgency but also by who has access to transplantation). Developing a medical urgency score with a
Cox proportional hazards model (which is how MELD 3.0 and GEMA-Na were developed)
is thus not suitable to repair any disparities which are the result of inequities
in transplantation access. Other methods are needed for this.

# Repeating the simulations

Above simulation was conducted only 1 time. We can run the simulation repeatedly
to confirm that this results holds in general.
